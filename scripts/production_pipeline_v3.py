"""
Production-Grade Rice Disease Pipeline v3
...
Usage:
  d:\Code\.venv310\Scripts\python scripts/production_pipeline_v3.py
"""
...
def build_model(lr=1e-3, dropout=0.45, l2=1e-4, train_base=False):
    ...
    m = tf.keras.Model(inp, out, name="rice_v3")
    m.compile(
...
def main():
    ...
    KERAS_OUT = args.models_dir / "rice_disease_model_v3_final.keras"
    TFLITE_OUT = args.models_dir / "rice_disease_model_v3_final.tflite"
    CAL_OUT = args.models_dir / "calibration_config_v3.json"
    REPORT_OUT = args.diag_dir / "production_eval_v3.json"
        ...
        h1 = model.fit(ds_train, validation_data=ds_val, epochs=args.stage1_epochs,
            class_weight=cw, callbacks=[
                ...
                tf.keras.callbacks.ModelCheckpoint(
                    str(args.models_dir/"v3_stage1_best.keras"),
    ...
    print("\n" + "="*60)
    print("PRODUCTION PIPELINE V3 COMPLETE")
    print("="*60)
